...
complex double function(complex double z){
  ...
  return (zbar*zbar*zbar-1)*(z*z*z-1);
}

complex double gradient(complex double z){
  complex double zbar = conj(z);
  return 6*zbar*zbar*(z*z*z-1);
}

complex double iterate(complex double z,double alpha){
  complex double res_z;
  res_z=z-gradient(z)*(alpha+0.0*I);
  return res_z;
}
...
for (y = height - 1; y >= 0; y--)     // BMP image format is written from bottom to top...
{
   for (x = 0; x <= width - 1; x++)
   {
      ...
      red = picture[x+width*y];
      green = picture[x+width*y];
      blue = picture[x+width*y];
    ...
    */
    ...
    for(y=0;y<1024;y++){
      for(x=0;x<1024;x++){
        ...
        niterations=0;
        while((mindistance(z,roots,NPOINTS)>1e-6)&&(niterations<1000)){
          z=iterate(z,0.1);
          niterations++;
        }
